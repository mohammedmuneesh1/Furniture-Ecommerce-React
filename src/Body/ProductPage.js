...
const ProductPage = () => {
  ...
  const { item, setItem ,cart,setCart,lstatus} = useContext(MyData);
    ...
    <>
      ...
      <div className="details d-flex flex-column flex-md-row align-items-center pb-3">
        ...
        <div className="d-flex flex-column w-100 w-md-50 text-black me-5 ms-5">
          <h1 className="fw-bold mb-3">{item[id - 1].name}</h1>
          <h4 className="fw-bold mb-3">₹{item[id - 1].price}</h4>
          <hr />
          <p className="mt-3 text-muted mb-4">{item[id - 1].description}</p>
